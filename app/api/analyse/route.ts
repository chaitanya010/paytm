...
import { buildPaytmSentiment } from '@/lib/competitiveAnalyzer'
import { saveReport, saveReportMem } from '@/lib/cache'
import { APPS, RELEVANCE_KEYWORDS } from '@/lib/constants'
import { AppId, Platform, TimeFilter, ScreenAnalysis, RawReview } from '@/types'
...
export async function POST(req: NextRequest) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      function emit(step: string, status: string, detail?: string, extra?: Record<string, unknown>) {
        if (closed) return
        const data = JSON.stringify({ step, status, detail, slug, ...extra })
      ...
      try {
        ...
        // ── Step 5: Save partial report (Paytm-only) ─────────────────────────
        const paytmSentiment = buildPaytmSentiment(paytmTagged, painPoints)
        const report = {
          slug,
        ...
        // ── Step 6: Hand off competitive work to client → /api/analyse/compete
        if (competitors.length > 0) {
          // Pre-filter competitor reviews so phase 2 doesn't need to re-filter
          const competitorRaw: Record<string, RawReview[]> = {}
          for (const cid of competitors) {
            ...
            competitorRaw[cid] = cFiltered.slice(0, 40)
          }

          // Sentiment stats (compact — avoids passing full TaggedReview array)
          const paytmStats = {
            total: paytmTagged.length,
            positive: paytmTagged.filter(r => r.sentiment === 'positive').length,
            negative: paytmTagged.filter(r => r.sentiment === 'negative').length,
            neutral:  paytmTagged.filter(r => r.sentiment === 'neutral').length,
          }

          emit('PHASE1_COMPLETE', 'done', undefined, {
            competitorRaw,
            paytmStats,
            focusArea,
            timeFilter,
